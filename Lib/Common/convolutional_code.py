...
class ConvolutionalCode:
    ...
    def __init__(self, generators: tuple):
        """
        :param generators: each element in the tuple represents a single generator polynomial. The convention we use is: x+1 =b011 = 3 (and not 1+D=6).
        """
    ...
    def decode(self, data: list[int]) -> (list[int], bytes, int):
        """
        decode data bytes
        :param data: coded data to be decoded, list of ints representing each received bit.
        :return: return bits, return a tuple of decoded data, and the amount of corrected errors.
        :rtype: (bits, bytes, int)

        The function assumes initial and final state of encoder was at the zero state
        """
        data.extend([0]*self.constraint_length*2)
        ...
        return decoded_bits, decoded_bytes, chosen_path.path_metric()
