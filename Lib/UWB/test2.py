function [SNR cber pber]= CSUWB_ABOVE_SPARSITY(crs,Np,Nf,CMN,num_channels)
%CSUWB_ABOVE_SPARSITY(cr,Np,Nf,ch,num_channel)
%CSUWB_ABOVE_SPARSITY: Compressed Sensing based Ultra-wideband system with
% sensing matrix operating above the sparsity level of 35%
% Inputs
%crs Compression Ratio (M/N), default [0.4 0.6]
...
%num_channel Number of channel realizations, default 1000
%Outputs
...
%Description
%CSUWB_ABOVE_SPARSITY gives BER performance graph for conventional
...
Ni=2; %Number of information symbols
Ns=Np+Ni; %Total number of symbols
SNR=[-17:1:-3]; % SNR in dB
%###########PULSE GENERATION ######################################
...
%############ UWB CHANNEL ##########################################
uwb_h=uwb_channel(CMN,num_channels);
%############# SIMULATION ###########################################
...
%################### PERFORMANCE PLOTS ####################################
figure,
semilogy(SNR,cber(1,1:snrdb),'k-*',SNR,pber(1,1:snrdb),'k-s',SNR,cber(2,1:snrdb),'k:+',SNR,pber(2,1:snrdb))
title('BER Performance for Compression Ratio(M/N) more than 35%')
xlabel('SNR (dB)')
ylabel('Average BER')
legend( 'CCS-UWB 40%', 'PCS-UWB 40%','CCS-UWB 60%', 'PCS-UWB 60%')
axis([-17 -4 .0001 1])
